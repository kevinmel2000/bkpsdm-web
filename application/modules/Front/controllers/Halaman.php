...
class Halaman extends CI_Controller {
	...
	public function __construct(){
		...
		public function berita($cUrl=""){	
			$dataHeader['title']	= "Tutorial | Sistem Informasi Panduan Budidaya Jamur";
			$dataHeader['menu']		= $this->model->ViewAsc('menu','id_menu');
			$dataHeader['link']		= "tutorial";	
			$data['apa']			= "berita";
			$data['judul']			= "Berita";
			$data['row']			= $this->model->ViewWhere('v_blog','id_kategori','3');
			
			$this->load->view('front-end/container/header',$dataHeader);
			$this->load->view('front-end/blog_berita',$data);
			$this->load->view('front-end/container/footer');
		}

		public function pembelajaran($cUrl=""){	
			$dataHeader['title']	= "Modul Pembelajaran | Sistem Informasi Panduan Budidaya Jamur";
			$dataHeader['menu']		= $this->model->ViewAsc('menu','id_menu');
			$dataHeader['link']		= "tutorial";	
			$data['apa']			= "pembelajaran";
			$data['judul']			= "Pembelajaran";
			$data['kelasmu']		= $this->input->post('cKelas');
			$data['mapelmu']		= $this->input->post('cMapel');
			$data['kelas']			= $this->model->ViewAsc('v_kelas','id_kelas');
			$data['mapel']			= $this->model->ViewAsc('mapel','id_mapel');
			if($cUrl == 'cari'){
				if (empty($this->input->post('cKelas')) && empty($this->input->post('cMapel'))) {
					$data['row']			= $this->model->ViewWhere('v_blog','id_kategori','1');
				} else {
					$data['row']		= $this->relasi->cariBlog($this->input->post('cKelas'), $this->input->post('cMapel'),'1');
				}
				
				/*$data['row']		= $this->model->ViewWhereAnd('v_blog','id_kelas',$this->input->post('cKelas'),'id_kategori','1');*/
			}else{
				$data['row']			= $this->model->ViewWhere('v_blog','id_kategori','1');
			}
		...
		public function modelpemebelajaran($cUrl=""){	
			$dataHeader['title']	= "Modul Model Pembelajaran | Sistem Informasi Panduan Budidaya Jamur";
			$dataHeader['menu']		= $this->model->ViewAsc('menu','id_menu');
			$dataHeader['link']		= "tutorial";	
			$data['apa']			= "modelpemebelajaran";
			$data['judul']			= "Model Pembelajaran";
			$data['kelasmu']		= $this->input->post('cKelas');
			$data['mapelmu']		= $this->input->post('cMapel');
			$data['kelas']			= $this->model->ViewAsc('v_kelas','id_kelas');
			$data['mapel']			= $this->model->ViewAsc('mapel','id_mapel');
			if($cUrl == 'cari'){
				if (empty($this->input->post('cKelas')) && empty($this->input->post('cMapel'))) {
					$data['row']			= $this->model->ViewWhere('v_blog','id_kategori','2');
				} else {
					$data['row']		= $this->relasi->cariBlog($this->input->post('cKelas'), $this->input->post('cMapel'),'2');
				}
			}else{
				$data['row']			= $this->model->ViewWhere('v_blog','id_kategori','2');
			}
		...
		public function blog_detail($cUrl=""){
			...
			foreach ($vaDataUrl as $key => $vaUrl) {
				...
				$cKategori 	 =  $vaUrl['nama_kategori'];
			}
			$dataHeader['title']	= "".$cJudul." | Sistem Informasi Panduan Budidaya Jamur";
			$dataHeader['link']		= "blog_detail";
			$data['judul']			= $cKategori ;
